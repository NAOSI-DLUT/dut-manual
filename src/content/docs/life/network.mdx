...
出于不明原因，大连理工大学校园网阻断了来自外部的 IPv4 NTP 流量，而大工自己的 NTP 服务器（time.dlut.edu.cn，IPv4 地址为 202.118.66.180）无论是 IPv4 还是 IPv6 连接下同步时间都是正常的。如要使用外部 NTP 服务器，需要有 IPv6 网络连接，或不使用大连理工大学校园网。
